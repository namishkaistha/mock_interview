...
def _mock_elevenlabs(mocker, status_code=200, content=b"fake-audio-bytes"):
...
def test_tts_returns_audio_stream(mocker):
    ...
    _mock_elevenlabs(mocker, status_code=200, content=b"fake-mp3-bytes")
...
def test_tts_empty_text_returns_422(mocker):
    """Empty text returns 422 without calling ElevenLabs."""
...
def test_tts_elevenlabs_error_returns_502(mocker):
    """ElevenLabs returning non-200 results in 502."""
    _mock_elevenlabs(mocker, status_code=401)
